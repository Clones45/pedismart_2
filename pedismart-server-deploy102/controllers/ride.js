  ...
  try {
    ...
    if (req.io) {
      ...
      if (ride.passengers && ride.passengers.length > 0 && (status === "ARRIVED" || status === "COMPLETED")) {
        ...
        // ONLY if the status was changed in this update
        ride.passengers.forEach(passenger => {
          // Check if passenger status matches the new auto-updated state
          // and they were either ONBOARD (for COMPLETION) or WAITING (for ARRIVAL)
          const wasWaitingAndNowOnboard = status === "ARRIVED" && passenger.status === "ONBOARD";
          const wasOnboardAndNowDropped = status === "COMPLETED" && passenger.status === "DROPPED";

          if (wasWaitingAndNowOnboard || wasOnboardAndNowDropped) {
            ...
            console.log(`👤 Skipping notification for passenger ${passenger.firstName} - status ${passenger.status} already set or irrelevant`);
          }
        });
      }
      ...
      if (riderSocket) {
        ...
        riderSocket.emit("passengerJoinRequest", {
          ...
          ride: {
            ...
            joinerPickup: req.body.pickup,
            joinerDrop: req.body.drop
          }
        });
